...
allowed-tools: browser_action read_current_page inspect_page observe_visual_page interact_page ask_user load_skill
---
...
5. 需要点击、翻页或打开明确目标时，先用 `inspect_page` 按名称或角色查找；需要浏览当前视口全部控件时用 `inspect_page` 的 `scope="viewport"`。然后把最新的 `observationId + ref` 交给 `interact_page`。
