...
//
// 把 `lib/speech/recognition.ts` 的纯逻辑包成 React 状态机，供 Composer 使用。
// 职责边界：
//  - 本 hook 只管「识别会话生命周期 + 状态」。麦克风授权的探测与引导由调用方
//    （Composer）用 `lib/speech/mic-permission.ts` 处理，再调用 `start()`——
//    所以 `start()` 假定已授权。
...
//    听写（无需用户再点）；下载失败则退到云端。
//  - 中间结果通过 `onInterim` 实时交给上层（Composer 把它作为编辑器末尾的
//    「未定稿后缀」实时上屏，因此输入框始终可编辑）；每段最终结果经
//    `cleanTranscript` 清洗后由 `onFinal` 回调定稿。
//
// 设计取舍：没有「语义校正」通道（即不修正识别文本的语义错误），onFinal 只做
// 空格清洗；空结果（清洗后为空串）不回调，避免上屏空文本。
...
export interface UseSpeechRecognitionOptions {
  ...
  /** 每段最终结果（已清洗），用于写入输入框定稿。 */
  onFinal: (text: string) => void;
  /** 归一化错误回调，用于提示/降级（如 not-allowed、language-unavailable）。 */
...
export function useSpeechRecognition(
  ...
  const start = useCallback(async () => {
    ...
    const handle = startSpeechSession(
      lang,
      {
        onInterim: (text) => {
          enqueue(() => {
        ...
        onFinal: (raw) => {
          enqueue(async () => {
            ...
            if (isCurrent() && cleaned) onFinalRef.current(cleaned);
          });
        },
        onError: (kind) => {
          if (!isCurrent()) return;
